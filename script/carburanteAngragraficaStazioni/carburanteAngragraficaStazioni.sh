...
if [ $code -eq 200 ]; then
  ...
  <"$folder"/../../docs/"$nome"/anagrafica_impianti_attivi.csv sed -r 's/"//g' | tail -n +2 | grep -vP '&#' | mlr --csv --ifs ";" clean-whitespace then sorf -f idImpianto >"$folder"/../../docs/"$nome"/anagrafica_impianti_attivi-cleaned.csv
